...
interface approvedstakeProps {
  isapprovedStakeOpen: boolean
  setisapprovedStakeOpen: (state: boolean) => void
}

const StakingDetailApproved: React.FC<approvedstakeProps> = ({ isapprovedStakeOpen, setisapprovedStakeOpen }) => {
  const [isdelModalOpen, setIsdelModalOpen] = useState(false)
  const onDelClick = () => {
    setIsdelModalOpen(true)
    setisapprovedStakeOpen(false)
  }
  const handleOk = () => {
    setisapprovedStakeOpen(false)
  }
  ...
  return (
    <>
      <Modal open={isapprovedStakeOpen} onOk={handleOk} onCancel={handleCancel} className="del-modal " centered width="415px">
        <div className="modal-content flex flex-col pt-[16px] pb-[24px] px-[18px] ">
          <p className="medium text-primary">
            Staking Pool details - <span className="text-green">(Approved)</span>
          </p>
...
export default StakingDetailApproved
