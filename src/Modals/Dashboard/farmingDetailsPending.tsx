...
interface pendingfarmProps {
  ispendingfarmOpen: boolean
  setispendingfarmOpen: (state: boolean) => void
}

const FarmingDetailPending: React.FC<pendingfarmProps> = ({ ispendingfarmOpen, setispendingfarmOpen }) => {
  const [isdelModalOpen, setIsdelModalOpen] = useState(false)
  const onDelClick = () => {
    setIsdelModalOpen(true)
    setispendingfarmOpen(false)
  }
  const handleOk = () => {
    setispendingfarmOpen(false)
  }
  ...
  return (
    <>
      <Modal open={ispendingfarmOpen} onOk={handleOk} onCancel={handleCancel} className="del-modal " centered width="415px">
        <div className="modal-content flex flex-col pt-[16px] pb-[24px] px-[18px] ">
          <p className="medium text-primary">
            Farming Pool details - <span className="text-txtPending">(Pending)</span>
          </p>
...
export default FarmingDetailPending
